...
class rescueMissionControl(Node):
    def __init__(self):
        ...
        self.mode_client = self.create_client(SetMode, '/plane/mavros/set_mode')
        self.wp_clear_client = self.create_client(WaypointClear, '/plane/mavros/mission/clear')
        self.wp_push_client = self.create_client(WaypointPush, '/plane/mavros/mission/push')
        self.command_int_client = self.create_client(CommandInt, '/plane/mavros/cmd/command_int')
        self.speed_client = self.create_client(CommandLong, '/plane/mavros/cmd/command')

        # ---------- Publishers ----------
        self.vel_pub = self.create_publisher(TwistStamped, '/plane/mavros/setpoint_velocity/cmd_vel', 10)
        self.pos_pub = self.create_publisher(PoseStamped, '/plane/mavros/setpoint_position/local', 10)
        self.global_pos_pub = self.create_publisher(GeoPoseStamped, '/plane/mavros/setpoint_position/global', 10)
        self.raw_global_pub = self.create_publisher(GlobalPositionTarget, '/plane/mavros/setpoint_raw/global', 10)

        # ---------- Subscribers ----------
        self.ext_state_sub = self.create_subscription(ExtendedState, "/plane/mavros/extended_state", self.ext_state_cb, qos_profile_sensor_data)
        self.state_sub = self.create_subscription(State, '/plane/mavros/state', self.state_cb, 10)
