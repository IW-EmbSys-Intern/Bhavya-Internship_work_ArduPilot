...
----------------------
TargetDetection.msg has no header/timestamp, and MAVROS does not publish a
...
class GeolocatorNode(Node):
    def __init__(self) -> None:
        ...
        # TargetDetection.msg that has no (or zero) header.stamp.
        ...
        self.create_subscription(
            NavSatFix, '/mavros/global_position/global', self.global_cb, sensor_qos)

        self.create_subscription(
            TargetDetection, '/detection/target', self.detection_cb, 10)

        # ---------------- Publisher -----------------------------------------
        self.pub_gps = self.create_publisher(TargetGPS, '/detection/target_gps', 10)
    ...
    # -------------------------------------------------------------------
    def detection_cb(self, msg: TargetDetection) -> None:
        ...
        # If TargetDetection.msg has a populated header.stamp (recommended:
